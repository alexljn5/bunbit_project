...
import { startMenuLoop, attachSettingsMenuHandlers, initFileInput } from "./settings/menusettings.js";
...
export function setupMenuClickHandler() {
    ...
    canvas.addEventListener("click", (e) => {
        console.log('[GameMenu] Canvas click handler fired, menuActive:', menuActive);
        if (!menuActive) {
            console.log('[GameMenu] Click ignored because menuActive is false');
            return;
        }
        ...
        const { x, y } = screenToCanvas(canvas, e);
        console.log('[GameMenu] Click at canvas coords:', x, y, 'buttons count:', buttons.length);

        if (showMapSelect) {
            console.log('[GameMenu] In map select mode');
        ...
        for (const b of buttons) {
            const isHit = hit(b, x, y);
            console.log('[GameMenu] Button:', b.name, 'at', b.x, b.y, b.w, b.h, 'hit:', isHit);
            if (!isHit) continue;

            console.log('[GameMenu] Button clicked:', b.name);
            ...
            if (b.name === "Settings") {
                console.log('[GameMenu] Settings button action starting');
                setMenuActive(true);
                startMenuLoop();
                attachSettingsMenuHandlers();
                initFileInput();
                console.log('[GameMenu] Settings button action complete');
            }
        ...
        console.log('[GameMenu] No button was hit');
    });
}
