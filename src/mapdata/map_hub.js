...
export const map_hub_sector1 = [
    ...
    [fullTileBrick, fullTileBrickEye, emptyTile, emptyTile, fullTileBrick],
    [fullTileBrick, emptyTile, emptyTile, emptyTile, fullTileBrick],
    [fullTileBrick, emptyTile, emptyTile, emptyTile, fullTileBrick],
    [fullTileBrick, emptyTile, emptyTile, emptyTile, fullTileBrick],
...
// Sector metadata
// ------------------------
export const map_hub_sectors = [
    { id: "sector1", data: map_hub_sector1, startX: 0, startY: 0, width: map_hub_sector1[0].length, height: map_hub_sector1.length },
    { id: "sector2", data: map_hub_sector2, startX: 5, startY: 0, width: map_hub_sector2[0].length, height: map_hub_sector2.length },
    { id: "sector3", data: map_hub_sector3, startX: 11, startY: 0, width: map_hub_sector3[0].length, height: map_hub_sector3.length },
];

// ------------------------
// Core grid setup
// ------------------------
const mapHeight = Math.max(...map_hub_sectors.map(s => s.startY + s.height));
const mapWidth = Math.max(...map_hub_sectors.map(s => s.startX + s.width));
export const map_hub = Array(mapHeight).fill().map(() => Array(mapWidth).fill(emptyTile));
map_hub_sectors.forEach(({ data, startY, startX }) => {
    for (let y = 0; y < data.length; y++) {
        for (let x = 0; x < data[0].length; x++) {
            map_hub[startY + y][startX + x] = data[y][x];
        }
    }
});
...
export const map_hub_data = {
    grid: map_hub,
    sectors: map_hub_sectors,
