## BUGS
Fix bug where Canvas rescale in DEBUG mode causes button click/touch misalignment (likely coordinate space mismatch between UI layer and scaled render surface)

## - PRIORITY. Try 31/06/2026 - 
Fix WASM integration in raycasting: current implementation still always falls back to JS path; verify worker init flag propagation, WASM status gating, and batchPoC execution path so WASM is actually selected and used
